...
describe("Ellipse", function () {
  describe("그룹에 속하지 않은 경우.", function () {
    ...
    it("rx와 ry가 다르면 GE 커맨드를 생성해야 한다.", function () {
      ...
      expect(result[1].params[0]).to.equal(
        String(Math.round(model.rx * 2 * ratio))
      );
      expect(result[1].params[1]).to.equal(
        String(Math.round(model.ry * 2 * ratio))
      );
    });
    ...
    it("fill이 true일 경우에는 border-thickness의 값은 rx와 ry중 작은값 / 2 로 변환한다.", function () {
      ...
      var borderThickness =
        result[1].command == "GE" ? result[1].params[2] : result[1].params[1];
